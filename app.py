...
from database import db_connect,user_reg,user_loginact,user_upload,user_viewimages,doc_reg,view_d,book_act,doc_act,view_r,accept_req,view_status
...
@app.route("/docregact", methods = ['GET','POST'])
def registeract1():
   if request.method == 'POST':    
      id="0"
      status = doc_reg(id,request.form['username'],request.form['password'],request.form['email'],request.form['location'],request.form['specialist'],request.form['exp'])
      if status == 1:
       return render_template("doc.html",m1="sucess")
      else:
       return render_template("docreg.html",m1="failed")
...
def bookact1():
   if request.method == 'POST':    
      id="0"
      status = book_act(id,request.form['doctor'],request.form['patient'],request.form['disease'])
      if status == 1:
       return render_template("vd.html",m1="sucess")
      else:
       return render_template("vd.html",m1="failed")
#--------------------------------------------Login-----------------------------------------------------
...
@app.route("/docact", methods=['GET', 'POST'])
def docact1():
    if request.method == 'POST':
        status = doc_act(request.form['username'], request.form['password'])
        print(status)
        if status == 1:
            session['username'] = request.form['username']                             
            return render_template("dochome.html", m1="sucess")
        else:
            return render_template("doc.html", m1="Login Failed")
#-------------------------------------------Upload Image----------------------------------
...
def book():
    ...
    return render_template("book2.html",m1="sucess",username=username,uname=uname)
...
def accept():
    ...
    status = accept_req(doctor,patient)
    if status == 1:
       return render_template("vr.html",m1="sucess")
...
training_data_quesans = open('C:/Users/Mrida/Documents/2023/skin11/CBIT.txt').read().splitlines()
